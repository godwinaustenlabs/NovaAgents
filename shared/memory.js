...
export class BufferMemory {
  ...
  async load(client_id, agent_id) {
    ...
    return { turns: data.turns.slice(-this.limitTurns), summary: data.summary || "" };
  }
  async save(client_id, agent_id, turn) {
    const k = _key(client_id, agent_id);
    const data = _RAM.get(k) || { turns: [], summary: "" };
    data.turns.push(turn);
    if (data.turns.length > this.limitTurns) data.turns = data.turns.slice(-this.limitTurns);
    _RAM.set(k, data);
  }
}

export class SummaryMemory extends BufferMemory {
  constructor(limitTurns = 20) { super(limitTurns); }
  ...
  async summarizeIfNeeded(client_id, agent_id) {
    ...
    const messages = [
      { role: "system", content: "You compress conversation into a concise, factual summary retaining entities, intents, commitments, and outcomes. No flowery prose." },
...
export class DynamicMemory extends SummaryMemory {
  ...
  async saveAndMaybeSummarize(client_id, agent_id, turn) {
    ...
    const approx = turns.reduce((acc, t) => acc + LLM.estimateTokens(t.content), 0);
